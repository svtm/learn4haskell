...
Before we a diving into the FP concepts, let's cover an important
...
For comparison, specific types in Haskell a with an uppercase
...
takeEven xs = go 0 [] xs
  where
    ...
    go _ acc [] = acc
    go n acc xs
      | mod n 2 == 0 = go (n+1) (acc ++ (take 1 xs)) (drop 1 xs)
      | otherwise = go (n+1) acc (drop 1 xs)
